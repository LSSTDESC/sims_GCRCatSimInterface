...
class InstanceCatalogWriter(object):
    ...
    def write_catalog(self, obsHistID, out_dir='.', fov=2, status_dir=None,
        ...
        if 'knots' in self.descqa_catalog and do_knots:
            ...
            cat.write_catalog(os.path.join(out_dir, knots_name), chunk_size=50000,
                              write_header=False)
        ...
        if self.sprinkler is False:
            ...
            if do_bulges:
                ...
                cat.lsstBandpassDict = self.bp_dict
                cat.photParams = self.phot_params
                cat.write_catalog(os.path.join(out_dir, cat_name), chunk_size=50000,
                                  write_header=False)
            ...
            if do_sprinkled:
                ...
                gal_cat.write_catalog(os.path.join(out_dir, gal_name), chunk_size=50000,
                                      write_header=False)
                if has_status_file:
                    with open(status_file, 'a') as out_file:
                        duration = (time.time()-t_start)/3600.0
