...
class hostImage(object):
	...
	def format_catalog(self, df_line, fits_file_name, image_dir):
		...
		cat_str = 'object %i %f %f %f %s %f 0 0 0 0 0 %s 0.01 0 CCM %f %f CCM %f %f\n' % (sys_id,
                                                                      ...
                                                                      df_line['sedFilepath'],
                                                                      df_line['redshift'],
                                                                      df_line[os.path.join(str(sys_id) + "_" + str(sys_magNorm) + "_" + gal_type + ".fits")],
                                                                      df_line['internalAv'],
                                                                      df_line['internalRv'],
                                                                      df_line['galacticAv'],
                                                                      df_line['galacticRv'])
	...
	# image_dir is the location of the FITS stamps 
	# input_cat is the list of FITS stamps to be read in
	# output_cat is the location of the output instance catalogs  
		host_df = pd.read_csv(input_cat)	
		ang_sep_list = []
		image_list = os.listdir(image_dir)
		image_ids = np.array([image_name.split('_')[0] for image_name in image_list], dtype=np.int)
		ang_sep = angularSeparation(host_df['raPhoSim_lens'], host_df['decPhoSim_lens'], self.ra, self.dec)
