...
if __name__ == "__main__":
    ...
    with open(out_name, 'w') as out_file:
        ...
        where_clause = 'WHERE ra>=%.2f AND ra<=%.2f ' % (ra_min, ra_max)
        where_clause += 'AND decl>=%.2f AND decl<=%.2f' % (dec_min, dec_max)
        ...
        for healpix in healpix_list:
            ...
            q = cat.get_quantities(q_names, native_filters=[healpix_filter],
                                   filters=[(lambda r: r<=r_mag_limit,
                                             'mag_r_lsst')])
            ...
            for ii in range(n_gal):
                ...
                out_file.write('%.10f, %.10f, ' % (ra_smeared[ii],
                                                   dec_smeared[ii]))
                for bp in 'ugrizy':
                    out_file.write('%.5f, %.3f, ' %
