...
class GenerateInstcat:
    ...
    def __call__(self, visit, dry_run=False):
        ...
        command = (f'time generateInstCat.py --config_file {self.config_file} '
        ...
        command = (f'time python {self.script_dir}/create_agn_ic.py '
                   f'--obs_db {self.opsim_db_file} '
                   f'--obs_id {visit} '
                   f'--agn_truth_cat {agn_truth_cat} '
                   f'--file_out {file_out}')
        self._subprocess_check_call(command, dry_run)
        command = f'gzip -f {file_out}'
        self._subprocess_check_call(command, dry_run)
        if not dry_run:
            phosim_cat_file = os.path.join(self.instcat_dir, visit_dir,
                                           f'phosim_cat_{visit}.txt')
            with open(phosim_cat_file, 'a') as output:
                output.write('.'.join((f'includeobj {file_out}', 'gz\n')))
        ...
        command = (f'time python {self.script_dir}/create_sne_ic.py '
                   f'--obs_db {self.opsim_db_file} '
                   f'--obs_id {visit} '
        ...
        command = (f'time python {self.script_dir}/create_lensed_host_ic.py '
                   f'--obs_db {self.opsim_db_file} '
                   f'--obs_id {visit} '
                   f'--fov {self.fov} '
...
if __name__ == '__main__':
    ...
    processes = args.processes if len(visits) > 1 else 1
